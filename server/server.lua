...
--[[
    TODO :
        1. Save Data For Player Owned Wagons
]]
---------------------------------------------
--------------------------------------------

local function isKeyInTable(t, k)
...
VORPcore.Callback.Register('bcc-wagons:stowWagonAction', function(source, cb, wagonNetId, wagonModel, action, data)
    local wagon = NetworkGetEntityFromNetworkId(wagonNetId)

    if action == "add" then
        if not wagonCanStoreMore(wagon, wagonModel) then 
            cb(true)
        else
            wagonInventoryAddItem(wagon, data)
            cb(false)
        end
    elseif action == "remove" then
        cb(wagonInventoryRemoveItem(wagon, data)) 
    elseif action == "query" then
        cb(wagonSelectAll(wagon))
    end
end)

function wagonInventoryAddItem(entity, itemDataToAdd)
    local statebag = Entity(entity).state

    local inventoryItems = statebag.WagonInventoryItems

    if not inventoryItems then
        inventoryItems = { }

        statebag.WagonInventoryItems = inventoryItems
    end

    -- TODO : TimeZone add here
    itemDataToAdd.added = os.date( "%b %d, %H:%M", os.time() + 6.5 * 60 * 60 )

    inventoryItems[#inventoryItems + 1] = itemDataToAdd

    -- Não replicar.
    statebag:set('WagonInventoryItems', inventoryItems, true)

    return true
end

function wagonInventoryRemoveItem(entity, index)
    local statebag = Entity(entity).state
    ...
    local toberemoved = inventoryItems[index]
...
function wagonCanStoreMore(entity, model)
    local statebag = Entity(entity).state
    ...
    local capacity = isKeyInTable(LoadoutCapacity, model)

    if capacity < 1 then return false end

    if #inventoryItems + 1 > capacity then return false end

    return true
end

function wagonSelectAll(entity)
    local statebag = Entity(entity).state
